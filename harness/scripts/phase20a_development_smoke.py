#!/usr/bin/env python3
"""Functional DEVELOPMENT smoke for EthBollingerMeanReversion-v1 (no PnL)."""
...
def first_ready_index(values: Sequence[float | None]) -> int | None:
...
def development_smoke(repo_root: Path = REPO_ROOT) -> dict[str, object]:
    ...
    closes = [candle.close for candle in primary.candles]
    bollinger_index = first_ready_index(bollinger(closes, 20, 2.0))
    adx_index = first_ready_index(adx(primary.candles, 14))
    if bollinger_index != 19 or adx_index != 27:
        raise RuntimeError(f"indicator readiness mismatch: {bollinger_index}/{adx_index}")
    holdout_reads = sum(is_holdout_range(start, end) for _, start, end in requested_ranges)
    return {
        "STRATEGY": "EthBollingerMeanReversion-v1",
        ...
        **counts,
        "BOLLINGER_FIRST_INDEX": bollinger_index,
        "ADX_FIRST_INDEX": adx_index,
        "BOLLINGER_PERIOD": 20,
        "BOLLINGER_STDDEV_MULTIPLIER": 2.0,
        "ADX_PERIOD": 14,
        "ADX_MAX": 20.0,
        "EDGE_PRESENT": "NOT_EVALUATED",
        "ABSOLUTE_VIABILITY_GATE": "NOT_EVALUATED",
