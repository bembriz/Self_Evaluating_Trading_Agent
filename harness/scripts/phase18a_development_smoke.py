#!/usr/bin/env python3
"""Functional DEVELOPMENT smoke for EmaRsiBtcContext-v1."""
...
def decision_counts(
    ...
    *,
    ...
    if candidate_buys + blocked_buys != baseline_buys:
        raise RuntimeError("BTC candidate changed decisions outside the BUY filter")
    return {
        "BASELINE_BUY_SIGNALS": baseline_buys,
        "BTC_CONTEXT_BUY_SIGNALS": candidate_buys,
...
def development_smoke(repo_root: Path = REPO_ROOT) -> dict[str, object]:
    ...
    if counts["BTC_BLOCKED_BUYS"] <= 0:
        raise RuntimeError("fixed smoke range contains no BTC-blocked BUY candidates")
    holdout_reads = sum(is_holdout_range(start, end) for _, start, end in requested_ranges)
    return {
        "STRATEGY": "EmaRsiBtcContext-v1",
