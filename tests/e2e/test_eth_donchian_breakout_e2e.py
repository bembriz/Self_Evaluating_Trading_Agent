"""Phase 19A DEVELOPMENT-only end-to-end breakout proof."""
...
@pytest.mark.e2e
def test_breakout_development_chain_and_holdout_pristine() -> None:
    ...
    probe = EthDonchianBreakout.from_adapters(adapter=adapter)
...
@pytest.mark.e2e
def test_breakout_spec_id_differs_from_frozen_strategies() -> None:
    resolver = ImportlibSourceResolver()
    breakout_identity = strategy_artifact_identity(strategy_definition(), resolver)
    ...
    assert spec_id(_spec(breakout_identity)) != spec_id(_spec(regime_identity))
