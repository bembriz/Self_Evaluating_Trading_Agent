"""Phase 18A integration with frozen adapters and unchanged lab runner."""
...
from lab.strategies.ema_rsi_btc_context import EmaRsiBtcContext
...
def test_adapter_range_mismatch_fails_closed() -> None:
    ...
    with pytest.raises(ValueError, match="aligned"):
        EmaRsiBtcContext.from_adapters(primary=primary, context=context)
...
def test_adapter_symbol_and_timeframe_mismatch_fail_closed() -> None:
    ...
    with pytest.raises(ValueError, match="aligned BTCUSDT"):
        EmaRsiBtcContext.from_adapters(primary=valid_primary, context=context)
...
def test_primary_timeframe_identity_mismatch_fails_closed(
    ...
    with pytest.raises(ValueError, match="aligned ETHUSDT"):
        EmaRsiBtcContext.from_adapters(
            primary=_with_range(primary, timeframe="1h"), context=context
        )
